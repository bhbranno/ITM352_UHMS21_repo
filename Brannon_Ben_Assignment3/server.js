...
// Borrowed and modified code from Lab 13, 14
// Inspired by code from Kimberly Matutina when she shared her screen on May 6
// Followed Professor Port's Screencast
var products = require('./public/products_data.json'); // set variable of products from product data to products
...
app.use(session({ secret: "ITM352 rocks!" })); // Start sessions
...
app.post('/process_login', function (request, response, next) {
    ...
    if (typeof user_data[username] != 'undefined') { // Check if username entered exists in user data
        if (user_data[username].password == request.body.password) { // Check if password entered matches password in user data
            ...
            location.href = "${'./products_display.html?product_key=Outdoor'}";
            </script>`;
...
app.post('/process_register', function (request, response, next) {
    ...
    if ((request.body.name.length > 30 || request.body.name.length < 1)) {
    ...
    if (errors.length > 0) {
        ...
        response.redirect('registration_page.html?' + qs.stringify(request.query));
    }
});
...
app.post("/update_cart", function (request, response) {
    ...
    if (haserrors == true) { // if there are errors, send error message
        message = "Invalid quantities. Cart not updated";
    ...
    ref_URL.searchParams.set("message", message); // get qs and add to qs
...
app.get("/logout", function (request, response) {
    logout_message = `<script>alert('${user_data[username].name} has successfully logged out!'); location.href="./index.html";</script>`;
    response.clearCookie('user_info');
    response.send(logout_message);
});
// ------ End User Logout ----- //
...
app.post('/completePurchase', function (request, response) {
    ...
    var mailOptions = {
        from: 'armysurplus@uhtest.com',
        to: email,
        subject: 'Your recent order from Ben\'s Army Surplus Store',
...
function isNonNegInt(q, return_errors = false) { // Checks if the values input are a positive integer
    ...
    if (q == '' || q == null) q = 0; // If the input is "blank", set the value to 0 
