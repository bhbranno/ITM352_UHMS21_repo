...
app.get('/set_cookie', function (request, response, next) {
    ...
    let my_name = 'Ben Brannon'
    response.cookie('my_name', 'Ben Brannon', {maxAge: 5000});
...
app.get('/use_cookie', function (request, response, next) {
    ...
    response.send(`Welcome to the Use Cookie page ${request.cookies['my_name']}`);
    } else {
...
app.post('/process_login', function (request, response, next) {
    ...
    request.session['last_login'] = Date().stringify;
    ...
    if (typeof user_data[username_entered] != 'undefined') {
        if (user_data[username_entered]['password'] == password_entered) {
            ...
            // If good, send to invoice and send a cookie
            response.cookie('username_cookie', username_entered);
...
app.post('/process_register', function (request, response, next) {
    ...
    // Make password a minimum of eight characters
